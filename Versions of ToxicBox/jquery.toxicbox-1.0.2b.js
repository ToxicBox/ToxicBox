...
width =(screen.width)/zoom;height = $(window).height();if((navigator.userAgent.indexOf("Android") + 1)||(navigator.userAgent.indexOf("Mobile") + 1)) {height =window.innerHeight;}
if(navigator.userAgent.indexOf("Presto") + 1) {width = window.innerWidth ;height = window.innerHeight;}
}, false);
...
function standart_setting_after_click_databox () {
    ...
    if (cw>width) {cw=width-(parseInt(current_databox.css('border-left-width')))-(parseInt(current_databox.css('border-right-width')))-(parseInt(current_databox.css('paddingLeft')))-(parseInt(current_databox.css('paddingLeft')))-ps*2;}   
