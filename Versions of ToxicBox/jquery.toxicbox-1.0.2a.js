...
if(ts){
ts.css({'left':0+'px','top':0+'px','margin-left':0+'px','margin-top':0+'px','left':((width/2)-((cw)/2+ps))+'px','top':((height/2+$(window).scrollTop())-((ch)/2+ps))+'px'});
...
function check_images(ct,is_single){
	...
	ts.prepend('<img src="'+$(cd).attr('href')+'" alt="" class="toxic_current toxic_hidden" style="max-width:'+(width-ps*2-10)+'px;max-height:'+height+'px;" /><div id="toxic_textarea" class="toxic_hidden" style="min-height:44px;"></div>');
	...
	fix_img_height(this);
    if(ct){	
	if (is_single!=1 ){ttx.css('width',(cw-ps*2-40)).html('<div id="toxic_arrow_carrier" style="width:'+(cw)+'px;"><div class="toxic_left toxic_larrow" style="width:'+((cw+20)/2-10)+'px;margin-left:-20px;" ><a href="#back"><img src="'+iu+'left.png" alt="" /></a></div><div class="toxic_left toxic_rarrow" style="width:'+((cw+20)/2-10)+'px;"><a href="#next"><img src="'+iu+'right.png" alt="" /></a></div></div><p class="toxic_center countofimages">'+cl.im+' '+cin+' '+cl.o+' '+ci+'</p><p class="toxic_center toxic_title">'+ct+'</p>');chk_arrow();}
	if (is_single==1 ){ttx.html('<p class="toxic_center toxic_title" style="width:'+(parseInt($(this).css('width'))-40)+'px;padding-top:4px;">'+ct+'</p>');}
    }else{
	if (is_single!=1 ){ttx.html('<p class="toxic_center countofimages" >'+cl.im+' '+cin+' '+cl.o+' '+ci+'</p>').prepend('<div id="toxic_arrow_carrier" style="width:'+(cw)+'px;"><div class="toxic_left toxic_larrow" style="width:'+((cw+20)/2-10)+'px;margin-left:-20px;" ><a href="#back"><img src="'+iu+'left.png" alt="" /></a></div><div class="toxic_left toxic_rarrow" style="width:'+((cw+20)/2-10)+'px;"><a href="#next"><img src="'+iu+'right.png" alt="" /></a></div></div>');chk_arrow();}
    ...
    ts.animate({width:cw, height:ch, marginLeft:-(cw/2-old_width/2), marginTop:(-height/2+(height-ch)/2+old_height/2)}, 500/sp, function() {tl.fadeOut(100, function(){$(this).remove();});ttx.css('width',(cw-40));$('#toxic_showbox .toxic_current, #toxic_showbox #toxic_textarea, #toxic_showbox #toxic_close').fadeIn(200/sp);});
	});}
function st_set_after_cl_img(is_single){
    var ct=$(cd).attr('title');
    $('body').append('<div id="toxic_dark" class="images_close"></div><div id="toxic_showbox" style="height:'+old_height+'px;width:'+old_width+'px;left:'+(width/2-ps-old_width/2)+'px;top:'+((height)/2-ps-old_height/2+$(window).scrollTop())+'px;padding:'+ps+'px;"></div>');
    image_start_load();
...
function standart_setting_after_click_databox () {
    ...
    $('body').append('<div id="toxic_dark" class="div_close"></div><div id="toxic_showbox" style="height:'+old_height+'px;width:'+old_width+'px;left:'+(width/2-ps-old_width/2)+'px;top:'+((height)/2-ps-old_height/2+$(window).scrollTop())+'px;padding:'+ps+'px;"></div>');
	var ts=$("#toxic_showbox");
    image_start_load();
	var tl=$('#toxic_loading');
...
function standart_setting_after_click_alert () {
    ...
    $('body').append('<div id="toxic_dark" class="images_close"></div><div id="toxic_showbox" class="alert" style="height:'+old_height+'px;width:'+old_width+'px;left:'+(width/2-ps-old_width/2)+'px;top:'+((height)/2-ps-old_height/2+$(window).scrollTop())+'px;padding:'+ps+'px;"></div>');
    var ts=$("#toxic_showbox");
...
function close_all (){old_width=parseInt($('#toxic_showbox').css('width'));old_height=parseInt($('#toxic_showbox').css('height'));$('#toxic_dark, #toxic_showbox, #toxic_loading').fadeOut(200/sp, function(){if($('#toxic_showbox').find('.div_close').length){$('body').append(current_databox);current_databox.css('display','none');}$('#toxic_dark, #toxic_showbox').remove();});return false;}
})();});