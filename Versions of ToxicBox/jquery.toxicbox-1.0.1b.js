...
function standart_setting_after_click_databox () {
    ...
    if ($(cd).attr('title')){ts.prepend('<h2 id="toxic_h2" style="width:'+cw+'px;margin-bottom:'+ps+'px;" class="toxic_hidden">'+$(cd).attr('title')+'</h2>');ch = parseInt($(current_databox).css( 'height')+$('#toxic_h2').css( 'height'))+40+(ps*2);}else{ch = parseInt($(current_databox).css( 'height'))+(ps)+22;}	
	ts.prepend('<a href="#index"><img id="toxic_close" class="toxic_hidden div_close" src="'+iu+'close.png" alt="" /></a>');
	ts.append(current_databox);
