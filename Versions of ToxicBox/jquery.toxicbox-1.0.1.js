...
function check_images(ct,is_single){
	...
	ts.prepend('<a href="#index"><img id="toxic_close" class="toxic_hidden images_close" src="'+iu+'close.png" alt="" /></a>');$('#toxic_close').css('margin-left',''+cw-20+'px').css('margin-top',''+(ch-21)+'px');
 b = Date.now();var res = b - a;alert( "Скрипт выполнялся <"+ res +"> ms." );//время отладка
...
function standart_setting_after_click_databox () {
	...
	ts.html(current_databox);
    if ($(cd).attr('title')){ts.prepend('<h2 id="toxic_h2" style="width:'+cw+'px;margin-bottom:'+ps+'px;" class="toxic_hidden">'+$(cd).attr('title')+'</h2>');ch = parseInt($(current_databox).css( 'height')+$('#toxic_h2').css( 'height'))+40+(ps*2);}else{ch = parseInt($(current_databox).css( 'height'))+40+(ps*2);}	
	ts.prepend('<a href="#index"><img id="toxic_close" class="toxic_hidden div_close" src="'+iu+'close.png" alt="" /></a>');
//b = Date.now();var res = b - a;alert( "Скрипт выполнялся <"+ res +"> ms." ); //время отладка
 ts.animate({width:cw, height:ch, marginLeft:-(cw/2-10), marginTop:(-height/2+(height-ch)/2+10)}, 500/sp, function() {$(current_databox).fadeIn(200/sp);$('#toxic_close.div_close').css('margin-left',''+cw-20+'px').css('margin-top',''+(ch-21)+'px').fadeIn(200/sp);$('#toxic_h2').fadeIn(200/sp);tl.fadeOut(100, function(){$(this).remove();});if ((parseInt(ts.css('top')))+(parseInt(ts.css('margin-top'))-$(window).scrollTop())<0){ts.css('margin-top','0').css('top',$(window).scrollTop());}});
    }
...
$( document ).on( "click", ".toxic_larrow a" ,    function() {a = Date.now();get_new_img(back_src);});	
$( document ).on( "click", ".toxic_rarrow a" ,    function() {a = Date.now();get_new_img(next_src);});
$( document ).on( "click", 'a[data-toxic="group"]', function() {a = Date.now();var is_single=0; cd=this; st_set_after_cl_img(is_single);return false;});
$( document ).on( "click", 'a[data-toxic="single"]',function() {a = Date.now();var is_single=1; cd=this; st_set_after_cl_img(is_single);return false;});	
$( document ).on( "click", '[data-toxic-databox]',function() {a = Date.now(); cd=this; standart_setting_after_click_databox(); return false;});  
$( document ).on( "click", '[data-toxic-alert]',function() {a = Date.now(); cd=this; standart_setting_after_click_alert(); return false;});  
