...
var cl; if (l=='ru') {cl=rl;} else {cl=el;}	
...
function check_images(ct){
	...
	var cin;
	function fix_img_height(correct){$(correct).css({'max-height':''+(height-ttx.height()-ps*2)+'px','width':'auto'});cw=$(correct).width();$(correct).css('width',cw);ch = $(correct).height();}
	...
	if (is_single==1 ){ttx.html('<p class="toxic_center toxic_title" style="width:'+($(this).width()-40)+'px;padding-top:4px;">'+ct+'</p>');}
    }else{
	...
	if (is_single==1 ){ttx.css('height',20+'px').css('min-height',0);}}
	fix_img_height(this);
	$('#toxic_showbox .toxic_rarrow, #toxic_showbox .toxic_larrow').css('width',''+((cw+20)/2-10)+'px');
    ch=parseInt(ttx.css( 'height'))+ch;
	ts.prepend('<a href="#index"><img id="toxic_close" class="toxic_hidden images_close" src="'+iu+'close.png" alt="" /></a>');$('#toxic_close').css({'margin-left':''+cw-20+'px','margin-top':''+(ch-20)+'px'});
...
function standart_setting_after_click_databox () {
    ...
    if (cw==width){ if (auto_width=='on') {cw=basic_width;$(current_databox).css('width',cw+'px');}else{cw=width-ps*2;}}
    if (cw>width) {cw=width-(parseInt(current_databox.css('border-left-width')))-(parseInt(current_databox.css('border-right-width')))-(parseInt(current_databox.css('paddingLeft')))-(parseInt(current_databox.css('paddingLeft')))-ps*2;current_databox.css({'width':cw+'px','height':'auto'}).find('iframe').attr('width',cw);}   
    if ($(cd).attr('title')){ts.prepend('<h2 id="toxic_h2" style="width:'+cw+'px;" class="toxic_hidden">'+$(cd).attr('title')+'</h2>');ch = $(current_databox).height()+parseInt($('#toxic_h2').css( 'height'))+20+(parseInt(current_databox.css('border-top-width')))+(parseInt(current_databox.css('border-bottom-width')))+(parseInt(current_databox.css('padding-top')))+(parseInt(current_databox.css('padding-bottom')));}else{ch = $(current_databox).height()+20+(parseInt(current_databox.css('border-top-width')))+(parseInt(current_databox.css('border-bottom-width')))+(parseInt(current_databox.css('padding-top')))+(parseInt(current_databox.css('padding-bottom')));}	
 	ts.animate({width:cw, height:ch, marginLeft:-(cw/2-old_width/2), marginTop:(-height/2+(height-ch)/2+old_height/2)}, 500/sp, function() {ts.append(current_databox);ts.append('<a href="#index"><img id="toxic_close" class="toxic_hidden div_close" src="'+iu+'close.png" alt="" /></a>').css({'min-height':ch+'px','height':'auto'});current_databox.fadeIn(200/sp);$('#toxic_close.div_close').css('margin-left',''+cw-20+'px').fadeIn(200/sp);$('#toxic_h2').fadeIn(200/sp);tl.fadeOut(100, function(){$(this).remove();});if ((parseInt(ts.css('top')))+(parseInt(ts.css('margin-top'))-$(window).scrollTop())<0){ts.css('margin-top','0').css('top',$(window).scrollTop());}});
	}
function standart_setting_after_click_alert () {
	...
	cw=tsp.width();
    ch=tsp.height()+20;
	if (cw>=width) {cw=width-ps*2;$("#toxic_showbox p.alert").css('max-width',cw+'px');ch=tsp.height()+20;}
